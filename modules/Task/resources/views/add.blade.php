...
@section('content')
    <div class="add-project w-100 d-flex justify-content-center mt-2 mb-4">
        ...
        <div class="add-form d-flex flex-column align-items-center">
            ...
            <div class="add-label w-100"> Create new task </div>
    
            <form method="POST" action="{{ route('user.tasks.postAdd') }}" class="d-flex flex-column w-100">  
    
                <div class="mb-3">
                    <label for="title"> {{ __('Title') }} </label>
    
                    <div>
                        <input id="title" type="text" class="form-control @error('title') is-invalid @enderror" name="title" value="{{ old('title') }}" required placeholder="Title of the task..." autocomplete="title" autofocus>
                        @error('title')
                    ...
                    <div class="status mb-3">
                        ...
                        <div>
                            <select name="status" id="status" class="form-select {{$errors->has('status')? 'is-invalid' :''}}">
                                ...
                                <option value="1" {{ old('status') == 1 ? 'selected' : false}} >To Do</option>
                                <option value="2" {{ old('status') == 2 ? 'selected' : false}} >In Progress</option>
                                <option value="3" {{ old('status') == 3 ? 'selected' : false}} >Done</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <select name="project_id" id="project_id" class="form-select {{$errors->has('project_id')? 'is-invalid' :''}}">
                            ...
                            @if (getProjects()->count() > 0)
                                @foreach (getProjects() as $project)
                                    <option value="{{ $project->id }}" {{ old('project_id') == $project->id ? 'selected' : false}} > {{ $project->name }}</option>
                                @endforeach                                
                            @endif
                        </select>
                        @error('priority')
